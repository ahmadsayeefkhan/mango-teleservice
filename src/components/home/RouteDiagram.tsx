...
type Path = { d: string; dir: 1 | -1; delay: number };

/**
 * Route diagram: YOUR NETWORK → MANGO CORE (IIG + ITC) → INTERNATIONAL PATHS → GLOBAL NETWORKS.
 * HTML nodes in a grid; an SVG overlay measures node positions and draws elbow connectors with
 * Mango signal dots travelling along them (GSAP MotionPath). Stacks vertically on small screens
 * (no connectors), mirroring the mobile design.
 */
export function RouteDiagram() {
  ...
  // Measure nodes and build connector paths (desktop only).
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const rel = (r: DOMRect) => ({ l: r.left - box.left, r: r.right - box.left, t: r.top - box.top, b: r.bottom - box.top, cy: r.top - box.top + r.height / 2 });
      ...
      el.querySelectorAll<HTMLElement>("[data-node=yours]").forEach((n) => {
        ...
        next.push({ d: elbow(r.r, r.cy, c.l, c.cy), dir: 1, delay: (k++ * 0.7) % 2.1 });
      });
      el.querySelectorAll<HTMLElement>("[data-node=path]").forEach((n) => {
        const r = rel(n.getBoundingClientRect());
        next.push({ d: elbow(c.r, c.cy, r.l, r.cy), dir: 1, delay: (k++ * 0.7) % 2.1 });
      });
      const pathsEls = Array.from(el.querySelectorAll<HTMLElement>("[data-node=path]"));
      el.querySelectorAll<HTMLElement>("[data-node=global]").forEach((n, i) => {
        const p = pathsEls[Math.min(i, pathsEls.length - 1)];
        if (!p) return;
        const pr = rel(p.getBoundingClientRect());
        const r = rel(n.getBoundingClientRect());
        next.push({ d: elbow(pr.r, pr.cy, r.l, r.cy), dir: 1, delay: (k++ * 0.7) % 2.1 });
      });
  ...
  // Animate the dots along the connectors — built on first sight (MotionPath loads on demand) and
  // paused while the diagram is off-screen, so the loops never run under the hero or the footer.
  useGSAP(
    (_, contextSafe) => {
      const s = svg.current;
  ...
  const col = network.columns;
  const node = (label: string, kind: string, extra?: string) => (
    <li
      key={label}
      data-node={kind}
      className={cn("flex items-center gap-2.5 rounded-lg border border-white/10 bg-ink/60 px-3.5 py-3 text-[13.5px] text-paper/90", extra)}
    >
  ...
  return (
    <div ref={root} className="relative rounded-3xl border border-white/10 bg-graphite/60 p-5 md:p-8">
      ...
      <div className="relative grid gap-6 lg:grid-cols-[1fr_1.15fr_1fr_1fr] lg:gap-10 xl:gap-14">
        {/* Core first on mobile (design), second on desktop */}
